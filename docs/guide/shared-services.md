...
teams' boards at once, where it is **one shared subtree** — not a copy.
...
- An edit on one board — adding a task, moving work to `done`, a run finishing — is the **same row**
  every other mounting board reads, so boards never drift.
- A connected repository is **synced once per org**, not once per workspace, so mounting a service
  on a second team's board adds no extra GitHub traffic.
...
A change to a shared service — run progress, a bootstrap, a notification — is **fanned out** to
