...
| `@cat-factory/delegation-github-actions` | A `DelegatedExecutor` over GitHub Actions, for a deployment plugging [its own implement loop](../extend/delegated-executors.md) in as the executor of a step. It solves the three problems that are nobody's workflow: `workflow_dispatch` returns no run id (it correlates by `run-name`), Actions' conclusions are not the platform's vocabulary, and a dispatched workflow declares no outputs (it reads the pull request off the work branch). |
